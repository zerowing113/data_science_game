...
export function MissionGuide({ onOpenFinal }: { onOpenFinal: () => void }) {
  ...
  if (!state.active) return <section className="mission-guide" aria-label="Practice mission"><h2>One question, six steps</h2><p>Follow a guided practice journey from exploring demand to stocking the shop. Start with working Python, then write more of the repairs yourself. Progress and experiments save automatically in this browser.</p><button className="run-button" onClick={() => dispatch({ type: 'start' })}>{completed ? 'Resume guided practice' : 'Start guided practice'}</button><p>Or use the workspaces below for free practice. Ready to apply your learning?</p><button className="secondary-button" onClick={onOpenFinal}>Open final challenge</button></section>;
