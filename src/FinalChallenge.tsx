...
import { FinalForecastWeek, FinalSplit, FinalEvaluation } from './FinalChallengeVisual';
import { FeatureTimingTimeline } from './FeatureTimingVisual';
...
export function FinalChallenge({ onBack, active }: { onBack: () => void; active: boolean }) {
  const mission = useMission();
  const [animatedSeed, setAnimatedSeed] = useState<number>();
  ...
  return <section className="final-challenge" aria-label="Final challenge">
    ...
    <nav className="final-stages" aria-label="Final challenge activities"><a href="#final-modeling" aria-current={!preview && !current ? 'step' : undefined}><span>1</span>Model the week</a><a href="#final-stocking" aria-current={preview && !current ? 'step' : undefined}><span>2</span>Commit stock</a><a href="#final-recaps" aria-current={current ? 'step' : undefined}><span>3</span>Review and reflect</a></nav>
    <div className="final-learning-workspace" id="final-modeling"><div className="final-input-panel">
    <FinalSplit history={challenge.history} future={challenge.future} revealed={!!current} />
    <FinalForecastWeek future={challenge.future} attempt={preview} dirty={!!dirty} revealed={!!current} />
    <details><summary>Inspect the forecast cutoff</summary><FeatureTimingTimeline forecastPeriod={`${challenge.future[0].date} through ${challenge.future.at(-1)!.date}`} /></details>
    ...
    </div><div className="final-code-panel"><span className="eyebrow">YOUR INDEPENDENT MODEL</span><h3>Write the forecast</h3>
    ...
    <section className="final-stock-panel" id="final-stocking" aria-label="Final stock commitment"><span className="eyebrow">FROM FORECAST TO SHELVES</span><h3>Plan the festival shelves</h3><p>Choose all seven daily orders before revealing demand. Your saved Python forecast is the source for the proposed stock.</p>
    ...
    {submissions.map((item) => <article key={item.seed} aria-label={`Challenge ${item.seed} results`} className="challenge-recap">
      ...
      <FinalEvaluation seed={item.seed} attempt={item.attempt} rows={item.rows} modelMae={item.modelMae} baselineMae={item.baselineMae} />
      <ShopPlayback decision={{ ...item.shop, number: item.seed, experiment: { number: item.attempt.number }, replay: false }} autoPlay={animatedSeed === item.seed} active={active} />
      <p>Forecast quality and shop profit answer different questions. Completion still requires a valid forecast that beats the baseline and all three reflections.</p>
      ...
      </div> : <div className="saved-final-reflections"><h4>Your explanations</h4><div><p><strong>Features:</strong> {item.reflection.features || 'Not provided'}</p><p><strong>Evaluation:</strong> {item.reflection.evaluation || 'Not provided'}</p><p><strong>Stocking:</strong> {item.reflection.stocking || 'Not provided'}</p></div></div>}
    </article>)}
    </div>
