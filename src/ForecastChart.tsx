...
export function ForecastChart({ predictions }: { predictions?: number[] }) {
  const width = 720;
  const height = 210;
  ...
  return <>
    <svg className="forecast-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={predictions ? 'Demand forecast chart: observed demand and seven predicted days' : 'Observed demand chart: 28 days of store history'}>
      ...
      {forecast && <>
        ...
        {predictions?.map((value, index) => <circle key={index} cx={x(index + 28)} cy={y(value)} r="4" fill="#1b654c"><title>{dayLabel(scenario.future[index].date)}: {value.toFixed(1)} mugs</title></circle>)}
      </>}
