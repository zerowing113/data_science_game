...
import { MissingDataVisual, MissingDataFailure } from './MissingDataVisual';
...
export function MissingDataLesson() {
  ...
  return <section className="results-card missing-data-lesson" aria-labelledby="missing-data-title">
    ...
    {(python.phase === 'error' || python.phase === 'unavailable') && <div className="error-box" role="alert"><strong>No new forecast or score.</strong><MissingDataFailure error={python.message} interrupted={python.phase === 'unavailable'} /></div>}
    ...
    {records.map((record) => <article className="missing-record" aria-label={`Missing-data run ${record.number}`} key={record.number}>
      ...
      {record.status !== 'completed' ? <><strong>{record.status === 'failed' ? 'Failed — no forecast or score' : 'Interrupted — no forecast or score'}</strong><MissingDataFailure error={record.error} interrupted={record.status === 'interrupted'} /></> : <>
        <div className="missing-success"><strong>Completed Python run {record.number} · predictions and score saved</strong><p>Preparation method is not automatically verified. The choice records your intention; inspect this run’s saved Python and output for what actually executed.</p></div>
