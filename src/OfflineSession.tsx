...
export function OfflineSession({ children }: { children: ReactNode }) {
  ...
  return <>
    {launcher && <aside className="offline-banner" aria-label="Offline game controls"><span>Offline edition · {launcher.version}</span><button ref={stopButton} className="text-button" onClick={() => setConfirming(true)}>Stop game</button>
      {confirming && <div role="alertdialog" aria-label="Stop the local game"><p>Stop the game on this computer? Check the save status before stopping. Saved progress resumes in this browser; any running Python will be interrupted.</p><button autoFocus className="secondary-button" disabled={stopping} onClick={() => { setConfirming(false); stopButton.current?.focus(); }}>Keep playing</button><button className="secondary-button" disabled={stopping} onClick={stop}>Stop and close session</button>{error && <p role="alert">{error}</p>}</div>}
    </aside>}
    {children}
  </>;
}
