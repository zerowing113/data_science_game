...
The packages are unsigned test builds. No Windows signing certificate, Developer ID signing, or Apple notarization has been configured. Before consumer distribution, verify freshly downloaded ZIPs through Explorer/Finder on both platforms, record OS prompts and browser versions, and establish signing/notarization if required. Do not treat bypassing system protections as a release procedure. GitHub artifacts remain subject to the repository's existing access policy; this work does not change repository visibility.

Ticket #12 owns persistent game state. Until it lands, refresh/restart loses session progress and the Stop game confirmation says so. The launcher's fixed origin is stable across normal starts and replacement versions; it deliberately reports an occupied port rather than silently moving to a different browser-storage origin. A matching running version is reused; a different version must be stopped first. Agree on browser-local versus launcher-managed storage in #12 before promising portability between browsers or machines. Save/resume integration remains an open criterion in #10.
