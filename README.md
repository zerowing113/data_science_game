...
Both lessons load Python only when opened and keep their own editor and experiment history for the page session. Their historical practice scores are separate from the upcoming-week forecast and its stocking decisions. The leakage lesson does not invent an upcoming error score when future demand is unknown. Guided practice consumes inspected evidence from these lessons; final-completion rules remain a later ticket.
...
Previously reached workspaces remain mounted, preserving editors, attempts, Python sessions, and saved outcomes when revisited or when switching to free practice. The guide retains the exact inspected source record for each completed step. Progress, hints, and records last only until reload. Completing all six steps marks the **practice journey** complete; it does not claim a final win, fresh unseen evaluation, or demonstrated learning. The final challenge/reflection remains #9.
